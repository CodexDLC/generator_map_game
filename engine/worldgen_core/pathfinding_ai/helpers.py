...
def terrain_factor_of(kind: str, terrain_factor: Optional[Dict[str, float]] = None) -> float:
    """Стоимость попадания в клетку данного типа."""
    tf = terrain_factor or DEFAULT_TERRAIN_FACTOR
    return tf.get(kind, 1.0)


def is_walkable(kind_grid, x: int, z: int, terrain_factor: Optional[Dict[str, float]] = None) -> bool:
...
def no_corner_cut(kind_grid, x: int, z: int, nx: int, nz: int,
                  terrain_factor: Optional[Dict[str, float]] = None) -> bool:
    """
    Запрет «резать углы»:
    ...
    if dx != 0 and dz != 0:  # диагональ
        ...
        if not (in_bounds(w, h, *a) and in_bounds(w, h, *b)):
            return False
        return is_walkable(kind_grid, *a, terrain_factor) and is_walkable(kind_grid, *b, terrain_factor)
    return True
