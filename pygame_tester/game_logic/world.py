...
class GameWorld:
    ...
    def update(self, dt: float):
        player_cx, player_cz = self.grid_spec.axial_to_chunk_coords(self.player.q, self.player.r)
        if (player_cx, player_cz) != self.last_player_chunk_pos:
            self.last_player_chunk_pos = (player_cx, player_cz)
            print(f"[GameWorld] Player entered new chunk ({player_cx}, {player_cz}). Updating grid...")
            self._update_surrounding_grid()

        self._handle_path_movement(dt)

    def _update_surrounding_grid(self):
        center_cx, center_cz = self.grid_spec.axial_to_chunk_coords(self.player.q, self.player.r)
        needed_chunks = set()
    ...
    def get_tile_at_axial(self, q: int, r: int) -> Dict:
        ...
        if not chunk_data:
            return {"surface": "void", "navigation": NAV_PASSABLE, "overlay": 0, "height": 0}
    ...
    def _get_stitched_grids_for_pathfinding(self) -> Tuple[Dict[str, np.ndarray], Tuple[int, int]]:
        ...
        stitched_surface = np.full((rows * chunk_size, cols * chunk_size), "void", dtype=object)
        stitched_nav = np.full((rows * chunk_size, cols * chunk_size), NAV_PASSABLE, dtype=object)
        stitched_height = np.zeros((rows * chunk_size, cols * chunk_size), dtype=np.float32)
        ...
        for (cx, cz), chunk_data in self._loaded_chunks.items():
            ...
            if chunk_data.get("surface"):
                stitched_surface[z_offset:z_offset + chunk_size, x_offset:x_offset + chunk_size] = chunk_data["surface"]
            if chunk_data.get("navigation"):
                stitched_nav[z_offset:z_offset + chunk_size, x_offset:x_offset + chunk_size] = chunk_data["navigation"]
            if chunk_data.get("height"):
                stitched_height[z_offset:z_offset + chunk_size, x_offset:x_offset + chunk_size] = chunk_data["height"]
        ...
        # Смещение в гексах
        offset_q, offset_r = self.grid_spec.world_to_axial(min_cx * self.grid_spec.chunk_size_m,
                                                           min_cz * self.grid_spec.chunk_size_m)
    ...
    def set_player_target(self, target_wx: float, target_wz: float):
        ...
        stitched_grids, (offset_q, offset_r) = self._get_stitched_grids_for_pathfinding()
        ...
        print(f"[Pathfinder] -> Global Start: ({start_q},{start_r}), Goal: ({goal_q},{goal_r})")
        print(f"[Pathfinder] -> Local Start: {local_start}, Goal: {local_goal} with offset ({offset_q}, {offset_r})")
        ...
        if path:
            ...
            print(f"[Pathfinder] -> SUCCESS: Path found with {len(self.player.path)} waypoints.")
        else:
            self.player.path = []
    ...
    def move_player_by(self, dq: int, dr: int):
        ...
        if tile_info.get("navigation") in [NAV_PASSABLE, "bridge"]:
            ...
            print(f"Movement blocked at ({next_q},{next_r}) by {tile_info.get('navigation')}")
