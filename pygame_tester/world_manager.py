...
class WorldManager:
    ...
    def get_chunk_data(self, cx: int, cz: int) -> Dict | None:
        ...
        try:
            ...
            if len(raw_bytes_h) != expected_size:
                print(
                    f"[Client] -> ERROR: Corrupted heightmap.r16 for chunk ({cx},{cz})"
                )
                self.cache[key] = None
                return None
            height_values = list(
                struct.unpack(f"<{CHUNK_SIZE * CHUNK_SIZE}H", raw_bytes_h)
            )
            ...
            height_grid_m = [
                [
                    (val / 65535.0) * max_h
                    for val in [
                        height_values[i * CHUNK_SIZE : (i + 1) * CHUNK_SIZE]
            ...
            nav_grid = [
                [NAV_PASSABLE for _ in range(CHUNK_SIZE)] for _ in range(CHUNK_SIZE)
            ]
    ...
    # --- НАЧАЛО ИЗМЕНЕНИЙ ---
    def load_raw_regional_layer(
        self, cx: int, cz: int, layer_name: str
    ) -> np.ndarray | None:
        """
        ...
        scx = (
            (cx + offset) // region_size
            if cx >= -offset
            else (cx - offset) // region_size
        )
        scz = (
            (cz + offset) // region_size
            if cz >= -offset
            else (cz - offset) // region_size
        )
        ...
        try:
            with np.load(file_path) as data:
                if layer_name not in data:
                    print(
                        f"--- WARN: Raw layer '{layer_name}' not found inside {file_path}"
                    )
                    return None
            ...
            print(
                f"--- ERROR: Failed to load raw layer '{layer_name}' from {file_path}: {e}"
            )
            return None
