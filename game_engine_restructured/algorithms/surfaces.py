# ==============================================================================
# Файл: game_engine_restructured/algorithms/surfaces.py
# Назначение: "Специалист" по нанесению геологических текстур (земля, песок, скалы)
#             на основе карты высот.
# ВЕРСИЯ 2.0: Исправлены все импорты.
# ==============================================================================
...
# Удобные функции для работы с numpy-массивами и ID
from ..core.constants import surface_fill, surface_set, nav_fill
from ..numerics.slope import compute_slope_mask
...
def classify_initial_terrain(surface_grid: np.ndarray, nav_grid: np.ndarray):
    ...
    """
    print("  -> [Surface] Шаг 1: Грунтовка (земля/проходимо)...")
...
def apply_slope_textures(height_grid: np.ndarray, surface_grid: np.ndarray, preset: Any):
    ...
    print("  -> [Surface] Шаг 2: Нанесение скал на крутые склоны...")
    ...
    if pixels_changed > 0:
        print(f"    -> Найдено {pixels_changed} пикселей скал.")
...
def apply_beach_sand(height_grid: np.ndarray, surface_grid: np.ndarray, preset: Any):
    ...
    print(f"  -> [Surface] Шаг 3: Создание пляжей до высоты {beach_upper_limit:.1f}м...")
    ...
    if pixels_changed > 0:
        print(f"    -> Найдено {pixels_changed} пикселей пляжа.")
