...
# ВЕРСИЯ 2.0: Работает напрямую с NumPy массивами ID.
# ==============================================================================
...
def write_raw_chunk(path_prefix: str, chunk_data: GenResult):
    ...
    # --- НАЧАЛО ИСПРАВЛЕНИЯ ---
    # Напрямую берем numpy-массивы, которые пришли из RegionProcessor
    surface_ids = chunk_data.layers.get("surface")
    nav_ids = chunk_data.layers.get("navigation")

    # Проверяем, что это действительно numpy-массивы, а не что-то другое
    if not isinstance(surface_ids, np.ndarray) or not isinstance(nav_ids, np.ndarray):
        ...
        # Попытка аварийного преобразования, если пришли строки (старый формат)
...
def read_raw_chunk(path_prefix: str) -> GenResult | None:
    ...
    try:
        ...
        with np.load(grid_path) as data:
            # Напрямую читаем numpy-массивы
            height = data["height"].tolist() # Высоту оставляем списком для совместимости
        ...
        return GenResult(
            ...
            layers={
                ...
                "surface": surface_ids, # <-- NumPy array
                "navigation": nav_ids, # <-- NumPy array
...
def write_raw_regional_layers(path: str, layers: Dict[str, np.ndarray], verbose: bool = False):
    ...
    tmp_path = path + ".tmp"
    try:
        with open(tmp_path, "wb") as f: np.savez_compressed(f, **layers)
        os.replace(tmp_path, path)
