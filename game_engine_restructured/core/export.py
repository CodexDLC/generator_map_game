...
def write_raw_chunk(path_prefix: str, chunk_data: GenResult):
    ...
    meta = {
        ...
        "grid_spec": dataclasses.asdict(chunk_data.grid_spec)
        if chunk_data.grid_spec
        else None,
        "stage_seeds": chunk_data.stage_seeds,
    }
    _atomic_write_json(meta_path, meta)
    height_grid = np.array(
        chunk_data.layers.get("height_q", {}).get("grid", []), dtype=np.float32
    )
    surface_str = chunk_data.layers.get("surface", [])
    surface_ids = np.array(
        [[SURFACE_KIND_TO_ID.get(kind, 0) for kind in row] for row in surface_str],
        dtype=np.uint8,
    )
    nav_str = chunk_data.layers.get("navigation", [])
    nav_ids = np.array(
        [[NAV_KIND_TO_ID.get(kind, 1) for kind in row] for row in nav_str],
        dtype=np.uint8,
    )
    ...
    with open(tmp_path, "wb") as f:
        np.savez_compressed(
            f, height=height_grid, surface=surface_ids, navigation=nav_ids
        )
    os.replace(tmp_path, grid_path)


def write_raw_regional_layers(
    path: str, layers: Dict[str, np.ndarray], verbose: bool = False
):
    """
...
def read_raw_chunk(path_prefix: str) -> GenResult | None:
    ...
    try:
        ...
        with np.load(grid_path) as grid_data:
            ...
            surface_str = [
                [SURFACE_ID_TO_KIND.get(int(id_val), "base_dirt") for id_val in row]
                for row in surface_ids
            ]
            nav_ids = grid_data["navigation"]
            nav_str = [
                [NAV_ID_TO_KIND.get(int(id_val), "obstacle_prop") for id_val in row]
                for row in nav_ids
            ]
        ...
        result = GenResult(
            ...
            grid_spec=grid_spec,
            stage_seeds=meta["stage_seeds"],
            layers={
                "height_q": {"grid": height_grid},
                "surface": surface_str,
                "navigation": nav_str,
                "overlay": [
                    [0 for _ in range(meta["size"])] for _ in range(meta["size"])
                ],
            },
        )
        return result
...
def write_world_meta_json(
    ...
    **kwargs,
    ...
    data = {
        ...
        "assets": {
            "url_pattern": f"/world/v2_hex/{world_id}/{{scx}}_{{scz}}/{{cx}}_{{cz}}/",
            "filenames": {
                "height": "height_rev{rev}.r16",
                "control": "control_rev{rev}.r32",
            },
        },
    }
