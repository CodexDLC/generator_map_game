...
def _stitch_region_maps(
    ...
    stitched_grids = {
        "surface": [[KIND_GROUND for _ in range(region_pixel_size)] for _ in range(region_pixel_size)],
        "navigation": [[NAV_PASSABLE for _ in range(region_pixel_size)] for _ in range(region_pixel_size)],
    }
...
def _create_water_cost_grid(nav_grid: List[List[str]], penalty: float = 50.0) -> List[List[float]]:
    ...
    for z in range(height):
        for x in range(width):
            if nav_grid[z][x] == NAV_WATER: continue
            is_near_water = False
            for dz in range(-1, 2):
                for dx in range(-1, 2):
                    if dx == 0 and dz == 0: continue
                    nx, nz = x + dx, z + dz
                    if 0 <= nx < width and 0 <= nz < height and nav_grid[nz][nx] == NAV_WATER:
                        is_near_water = True
                        break
                if is_near_water: break
            if is_near_water: cost_grid[z][x] = penalty
    return cost_grid
...
def _build_global_mst(points: List[GlobalCoord]) -> List[Tuple[int, int]]:
    ...
    for _ in range(n):
        v = -1
        for i in range(n):
            if not used[i] and (v == -1 or min_e[i] < min_e[v]): v = i
        if v == -1: break
        used[v] = True
        if sel_e[v] != -1: edges.append((sel_e[v], v))
        for to in range(n):
            dist = _l1(points[v], points[to])
            if not used[to] and dist < min_e[to]: min_e[to], sel_e[to] = dist, v
    return edges
...
def plan_roads_for_region(scx: int, scz: int, seed: int, preset: Preset, base_chunks: Dict[Tuple[int, int], GenResult],
    ...
    if scx == 0 and scz == 0:
        ...
        for direction, end_pos in end_points.items():
            ...
            path = astar_find(
                stitched_surface_grid, stitched_nav_grid, None,
                start_pos, end_pos, policy=road_policy, cost_grid=water_cost_grid
            )
            if path:
