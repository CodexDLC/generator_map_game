...
class GameWorld:
    ...
    def get_tile_at(self, wx: int, wz: int) -> Dict:
        ...
        return {
            ...
            "overlay": overlay_grid[lz][lx] if (0 <= lz < len(overlay_grid) and 0 <= lx < len(overlay_grid[0])) else 0,
            # <-- Добавлено
    ...
    def set_player_target(self, target_wx: int, target_wz: int):
        ...
        stitched_overlay_grid = [[0 for _ in range(stitched_width)] for _ in range(stitched_height)]
        ...
        for cz_offset in range(num_chunks_z):
            for cx_offset in range(num_chunks_x):
                ...
                if chunk_data:
                    ...
                    if not surface or not nav or not height or not overlay: continue
                    for z in range(CHUNK_SIZE):
                        for x in range(CHUNK_SIZE):
                            ...
                            stitched_overlay_grid[paste_z_start + z][paste_x_start + x] = overlay[z][x]
        ...
        stitched_path = find_path(
            stitched_surface_grid, stitched_nav_grid, stitched_height_grid,
            start_stitched, goal_stitched
        )
