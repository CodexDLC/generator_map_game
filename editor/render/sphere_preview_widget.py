...
void main() {
    ...
    v_normal_view = mat3(transpose(inverse(model_view))) * normalize(aPos);
    v_color = aColor;
}
"""
...
out vec4 FragColor;
in vec3 v_color;
uniform bool u_is_line; // Флаг, чтобы рисовать линии черным

void main()
{
    if (u_is_line) {
        FragColor = vec4(0.0, 0.0, 0.0, 1.0); // Линии всегда черные
    } else {
        FragColor = vec4(v_color, 1.0); // Цвет полигона берется напрямую из данных
    }
}
"""
...
class SpherePreviewWidget(QOpenGLWidget):
    ...
    def initializeGL(self):
        ...
        # Включаем тест глубины (чтобы ближнее перекрывало дальнее)
        GL.glEnable(GL_DEPTH_TEST)
        # Включаем отсечение задних граней (чтобы не видеть шар "насквозь")
        GL.glEnable(GL_CULL_FACE)
        # Включаем сдвиг полигонов вглубь, чтобы линии рисовались поверх
    ...
    def paintGL(self):
        ...
        GL.glUniformMatrix4fv(GL.glGetUniformLocation(self._shader_program, "model_view"), 1, GL_FALSE, view.data())

        s = self._render_settings
        az_rad = math.radians(s.light_azimuth_deg);
        alt_rad = math.radians(s.light_altitude_deg)
        light_dir = [math.cos(alt_rad) * math.sin(az_rad), math.cos(alt_rad) * math.cos(az_rad), math.sin(alt_rad)]
        light_dir_view = view.mapVector(QtGui.QVector3D(*light_dir)).normalized()
        ...
        # 1. Рисуем заполненные полигоны шара (они будут сдвинуты вглубь благодаря glPolygonOffset)
        ...
        # 2. Рисуем черные линии гексов. Тест глубины включен, и они окажутся поверх полигонов.
