# editor/nodes/universal/math/normalize01_node.py
# ВЕРСИЯ 2.1: Исправлена регистрация свойств для корректной работы с типами.
...
class Normalize01Node(GeneratorNode):
    ...
    def __init__(self):
        ...
        # --- НАЧАЛО ИСПРАВЛЕНИЯ: Используем правильные методы и выставляем метаданные ПОСЛЕ ---
        self.add_text_input('mode', "Mode (auto/minmax/symmetric...)", tab="Normalize", text="auto")
        ...
        self.add_float_input('p_low', "Percentile Low (0..100)", tab="Percentile", value=1.0)
        self._prop_meta["p_low"]['group'] = "Percentile"

        self.add_float_input('p_high', "Percentile High (0..100)", tab="Percentile", value=99.0)
        self._prop_meta["p_high"]['group'] = "Percentile"

        self.add_checkbox('clip_after', "Clip to [0..1] after", tab="Output", state=True)

        # Для 'decimals' (int) используем add_text_input, но СРАЗУ исправляем тип в метаданных
        self.add_text_input('decimals', "Round Decimals (0=no round)", tab="Output", text="0")
        self._prop_meta["decimals"]['type'] = 'int'
        self._prop_meta["decimals"]['group'] = "Output"

        self.add_float_input('nan_fill', "NaN Fill", tab="Safety", value=0.0)
        self._prop_meta["nan_fill"]['group'] = "Safety"

        self.add_float_input('fill_const', "Fallback Const (flat)", tab="Safety", value=0.0)
        self._prop_meta["fill_const"]['group'] = "Safety"
        # --- КОНЕЦ ИСПРАВЛЕНИЯ ---
    ...
    def _compute(self, context):
        ...
        # Теперь get_property будет возвращать правильные типы
        ...
        # Блок try/except для совместимости со старыми версиями numpy/numba
        # остаётся без изменений
        try:
            # np.percentile в некоторых версиях Numba не поддерживает kwargs
            if mode == "percentile":
