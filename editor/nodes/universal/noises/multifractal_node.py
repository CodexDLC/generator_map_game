...
class MultiFractalNode(GeneratorNode):
    ...
    def __init__(self):
        ...
        self.add_float_input("scale", "Scale (0..1)", value=0.5, group="Fractal", p_range=(0.0, 1.0), p_widget='slider')
        self.add_text_input("octaves", "Octaves", group="Fractal", text="8")
        self.add_float_input("roughness", "Roughness (0..1)", value=0.5, group="Fractal", p_range=(0.0, 1.0), p_widget='slider')
        self.add_seed_input("seed", "Seed", group="Fractal")

        # --- ИЗМЕНЕНИЕ: Полностью переделанная секция Variation ---
        # --- Variation ---
        self.add_float_input("var_strength", "Variation", value=1.0, group="Variation", p_range=(0.0, 4.0), p_widget='slider')
        self.add_float_input("var_smoothness", "Smoothness", value=0.0, group="Variation", p_range=(-20.0, 20.0), p_widget='slider')
        self.add_float_input("var_contrast", "Contrast", value=0.3, group="Variation", p_range=(0.0, 1.0), p_widget='slider')
        self.add_float_input("var_damping", "Damping (0..1)", value=0.25, group="Variation", p_range=(0.0, 1.0), p_widget='slider')
        self.add_float_input("var_bias", "Bias (-1..1)", value=0.5, group="Variation", p_range=(0.0, 1.0), p_widget='slider')
        # --- КОНЕЦ ИЗМЕНЕНИЯ ---
        ...
        self.add_float_input("warp_freq", "Frequency", group="Warp", value=0.05, p_range=(0.0, 1.0), p_widget='slider')
        self.add_float_input("warp_amp", "Amplitude (0..1)", group="Warp", value=0.5, p_range=(0.0, 1.0), p_widget='slider')
